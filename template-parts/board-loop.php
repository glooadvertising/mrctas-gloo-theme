...
while ($volunteers->have_posts()) {
    ...
    <details id="meet-<?php echo basename(get_permalink()); ?>" class="cursor-pointer mb-6"><?php ?>
        <summary class="px-10 py-6 flex bg-gray-100 justify-between"><h3 class="font-extrabold"><?php the_title(); ?><small class="ml-4 font-light text-mrct-navy-light"><?php $custom = get_post_custom(); if(isset($custom['board_member_role'])) { echo $custom['board_member_role'][0];} else { the_title();}?></small></h3></summary>
